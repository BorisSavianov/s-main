// apps/scheduler-service/src/scheduling/guards/service-auth.guard.ts
import {
  Injectable,
  CanActivate,
  ExecutionContext,
...
export class ServiceAuthGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    this.logger.debug(`Internal service call: ${isInternal}`);
    ...
    if (serviceName && this.allowedServices.includes(serviceName)) {
      ...
      // If a userId is provided, we simulate a user object for decorators like @GetUser
      if (userId) {
        request.user = {
          id: userId,
          service: serviceName,
